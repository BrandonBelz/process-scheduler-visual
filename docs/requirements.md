...
- Required: included in the December baseline unless explicitly marked as stretch or design direction.
...
| --- | --- | --- |
| Program | Configuration with an ID, 1-20 required CPU cycles, I/O enabled flag, and I/O interval/duration each 1-10 cycles inclusive. All cycle inputs are whole numbers. Produces one process per policy instance. | Arrival-delay limits only if that stretch feature is implemented. |
| Process | Independent runtime instance of a program within a policy instance, with its own state and stateHistory. References its program. |  |
| Policy | Scheduling behavior with an ID, name, description, preemptive boolean, initialState, and scheduler. |  |
| Policy instance | One configured selection of a policy type, with independent settings and state. Duplicate types/settings are allowed and count toward the five-instance limit. Users can edit, remove, and reorder instances. | Distinguish using labels such as `RR #1` |
| Scheduler | Selects a runnable process for the next clock cycle, or indicates idle when none is runnable, using processes and policy-specific context. |  |
| Workload | The entire data set (policies, programs, configurations) that is simulated. Returning from configuration recomputes every run and resets playback. |  |
| Clock cycle | Discrete simulation time step, independent of playback speed; each process history has an entry for every global cycle. | Boundary conventions are specified in section 5. |
...
| PC-003 | Reserve January-April 2027 for testing and fixes; no new features are planned during that period. | Confirmed; exact deadline TBD |
| PC-004 | Deliver a deployed app, documentation, requirements document, and use cases. | Confirmed |
...
| FR-011 | Provide speed adjustment, pause/resume, next/previous cycle, restart, and jump to a selected cycle. | Required | Speeds are 1, 2, 3, and 4 cycles per second. Automatically pause at the final shared position; navigation cannot go before cycle 0 or beyond the end. Playback does not alter computed schedules. |
...
| FR-013 | Compute full histories and metrics before animated playback. | Required | Compute locally for every instance, then start paused at cycle 0; calculation is separate from when results become visible. |
| FR-014 | Configure whole-number CPU duration 1-20 and optional I/O interval/duration each 1-10 inclusive. | Required | Reject invalid active fields; disabled I/O produces no blocking. Waiting/blocked cycles do not reduce required CPU work. |
...
| FR-016 | Show brief explanations of scheduling events or decisions. | Stretch before January | Define event coverage if selected for implementation; not required for MVP acceptance. |
| FR-017 | Show the final results table at the end of playback. | Required December MVP | Reveal only when shared playback reaches the final position across all runs, including by jumping to the end. Identify instances and show per-process and mean turnaround/response times. |
...
| FR-019 | Calculate per-process and mean turnaround/response time for each policy instance. | Required | Use section 5; include each process exactly once. Do not display final metrics before playback ends. |
| FR-022 | Show a small, valid prefilled workload on a fresh visit. | Required | Include editable programs and at least one required policy instance. Exact example values are an implementation choice. |
| FR-023 | Require 1-10 programs and 1-5 policy instances to run, counting duplicates. | Required | Prevent an eleventh program or sixth instance; prevent running an empty configuration and explain the limits. |
| FR-024 | Retain configuration across in-app navigation; reset after browser refresh. | Required | Retain programs, settings, and order when navigating; refresh restores the initial example rather than prior user edits. |
| FR-025 | Provide editable policy defaults. | Required | RR defaults to a two-cycle slice, editable in whole numbers from 1-20. The other required policies have no additional agreed numeric parameters. |
...
| FR-029 | Allow editing, removing, and reordering policy instances. | Required | Reordering changes display order only. Distinguish duplicate instances by number and relevant settings, such as `RR #2 - slice 5`. |
| FR-030 | Validate inputs with messages beside the affected fields. | Required | Missing required, nonnumeric, fractional, and out-of-range values prevent running until corrected. Retain editable inputs and explain each correction; inactive I/O fields must not block running. |
...
| NFR-011 | Provide detailed deployment and maintenance documentation for professor ownership after April. | Required | Follow setup/build/test/deployment from a clean checkout; complete a professor or teammate walkthrough and correct missing instructions before handoff. |
...
The professor maintains the app after April. Deliver detailed documentation sufficient to work without the graduating team's memory:

1. **User guide:** inputs/limits, example, program/policy management, both views, controls, legend, metrics, refresh behavior, and troubleshooting.
2. **Developer guide:** repository structure, prerequisites and exact runtime/package-manager versions, installation, local startup, architecture/data flow, domain model, policy behavior, and timing conventions.
3. **Testing guide:** commands, reference traces, adding tests, browser checks, performance hardware/procedure, and known limitations.
4. **Chosen-host deployment guide:** account/access prerequisites, build command, output directory, required settings or their absence, routes/base paths, publishing, smoke checks, updates, and rollback/recovery.
5. **Maintenance guide:** adding/changing policies, updating dependencies, checking regressions, diagnosing failures, maintaining examples, and recording requirement changes.
6. **Ownership handoff:** repository/hosting administration, deployment permissions, release identifiers, known issues, unfinished stretch items, and course-artifact locations. Transfer access through account mechanisms; do not put credentials in documentation.

Delivery includes the deployed app and documentation for the selected environment, not merely a local build.
